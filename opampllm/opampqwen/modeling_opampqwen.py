...
from .configuration_opampqwen import OpampQwen2Config

logger = logging.get_logger(__name__)

_CONFIG_FOR_DOC = "OpampQwen2Config"
...
class Qwen2RotaryEmbedding(nn.Module):
    def __init__(
        ...
        scaling_factor=1.0,
        rope_type="default",
        config: Optional[OpampQwen2Config] = None,
    ):
        super().__init__()
...
class Qwen2Attention(nn.Module):
    ...
    def __init__(self, config: OpampQwen2Config, layer_idx: Optional[int] = None):
        super().__init__()
        self.config = config
        ...
        if self.layer_idx % self.adapter_layer == 0: 
            ...
            self.k1_adapter = OpampAdapterMLP(config, input_size=self.num_heads * self.head_dim)
            
            self.q2_adapter = OpampAdapterMLP(config, input_size=self.num_heads * self.head_dim)
            self.k2_adapter = OpampAdapterMLP(config, input_size=self.num_heads * self.head_dim)
    ...
    def forward(
        ...
        if self.layer_idx % self.adapter_layer == 0:    
            ...
            q1_states = q1_states.view(bsz, q_len, -1, self.head_dim).transpose(1, 2)
            k1_states = k1_states.view(bsz, k_len, -1, self.head_dim).transpose(1, 2)

            q2_states = q2_states.view(bsz, q_len, -1, self.head_dim).transpose(1, 2)
            k2_states = k2_states.view(bsz, k_len, -1, self.head_dim).transpose(1, 2)
...
class Qwen2FlashAttention2(Qwen2Attention):
    ...
    def forward(
        ...
        if self.layer_idx % self.adapter_layer == 0:    
            ...
            q1_states = q1_states.view(bsz, q_len, -1, self.head_dim)
            k1_states = k1_states.view(bsz, k_len, -1, self.head_dim)

            q2_states = q2_states.view(bsz, q_len, -1, self.head_dim)
            k2_states = k2_states.view(bsz, k_len, -1, self.head_dim)
...
class Qwen2DecoderLayer(nn.Module):
    def __init__(self, config: OpampQwen2Config, layer_idx: int):
        super().__init__()
        self.hidden_size = config.hidden_size
...
class Qwen2Model(Qwen2PreTrainedModel):
    ...
    def __init__(self, config: OpampQwen2Config):
