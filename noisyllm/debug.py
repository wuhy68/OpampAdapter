...
# from .configuration_noisyllama import NoisyLlamaConfig
...
config = NoisyLlamaConfig(num_hidden_layers=2, vocab_size=320)
config._attn_implementation = "flash_attention_2" ## "eager", "sdpa"
config.adapter_rank = config.head_dim # flash_attn only support the same head_dim for efficient training
print(config._attn_implementation)

NoisyLlama = LlamaForCausalLM(config).to(torch.bfloat16)
NoisyLlama = NoisyLlama.cuda()

hidden_states = torch.randn(4, 17, 4096).to(torch.bfloat16).cuda()
original_output = NoisyLlama(inputs_embeds=hidden_states)
